    ...
    <div class="content">
        ...
        <div class="table">
            <el-table align="center" v-bind:data="needList" border style="width:1228px;margin:auto" max-height="500" @selection-change="handleSelectionChange" :v-loading.body="loading">
                ...
                <el-table-column prop="breedName" label="药材名称" width="120">
                </el-table-column>
                ...
                <el-table-column prop="location" label="产地" width="100">
                </el-table-column>
                ...
                <el-table-column label="求购类型" width="120">
                    <template scope="scope">
                        ...
                        <span v-if="scope.row.indentType === 1">普通求购</span>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        sort(item, subItem) {
            this.httpParam.pn = 1;
            if (item.param != 'sampling' || item.param != 'indentType') {
                this.httpParam.sort[item.param] = subItem.key;
            } else {
                this.httpParam[item.param] = subItem.key;
            }
            if (item.param == 'indentType') {
                this.httpParam.indentType = subItem.key;
            }
            this.getHttp();
        },
        getHttp() {
